...
{
    public class ApprovedHourExtraCommand : IRequest<Result>
    {
        public ApprovedSalaryDto payload { get; set; }
    }

    public class ApprovedHourExtraCommandHandler : IRequestHandler<ApprovedHourExtraCommand, Result>
    {
        ...
        public ApprovedHourExtraCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(ApprovedHourExtraCommand request, CancellationToken cancellationToken)
        {
            ...
            if (approver != null)
            {
                ...
                await _staffRequestRepository.ApprovedHourExtra(request.payload, false);
                foreach (var item in list)
                {
                    Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

                    string fmt = File.ReadAllText(_appSettings.AcepteRequestTypeSure);
            ...
            {
                await _staffRequestRepository.ApprovedHourExtra(request.payload, true);

                Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

                string fmt = File.ReadAllText(_appSettings.ApprovedRequestMedico);
