...
{
    public class RejectRequestTypeSureCommand : IRequest<Result>
    {
        public RejectSalaryDto payload { get; set; }
    }

    public class RejectRequestTypeSureCommandHandler : IRequestHandler<RejectRequestTypeSureCommand, Result>
    {
        ...
        public RejectRequestTypeSureCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectRequestTypeSureCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectTypeSure(request.payload);
