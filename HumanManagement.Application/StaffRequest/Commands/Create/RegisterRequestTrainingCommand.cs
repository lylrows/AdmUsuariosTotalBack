...
{
    public class RegisterRequestTrainingCommand : IRequest<Result>
    {
        public RegisterCapacitacionDto payload { get; set; }
    }
    public class RegisterRequestTrainingCommandHandler : IRequestHandler<RegisterRequestTrainingCommand, Result>
    {
        ...
        public RegisterRequestTrainingCommandHandler(IBaseRepository<StaffRequestModel> baseStaffRequestRepository,
        ...
        public async Task<Result> Handle(RegisterRequestTrainingCommand request, CancellationToken cancellationToken)
        {
            try
            {
                
                if(request.payload.nid_request > 0)
                {
                    await _staffRequestRepository.RegisterRequestCapacitacionNueva(request.payload);
                    return new Result
                    {
                ...
                foreach (var item in list)
                {
                    ...
                    newNotification.IdArea = approver.IdArea;
