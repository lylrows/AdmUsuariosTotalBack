...
{
    public class RejectStaffRequesHourExtraCommand : IRequest<Result>
    {
        public RejectSalaryDto payload { get; set; }
    }

    public class RejectStaffRequesHourExtraCommandHandler : IRequestHandler<RejectStaffRequesHourExtraCommand, Result>
    {
        ...
        public RejectStaffRequesHourExtraCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectStaffRequesHourExtraCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectHourExtra(request.payload);
