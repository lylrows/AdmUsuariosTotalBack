...
{
    public class RejectTrainingExtraCommand : IRequest<Result>
    {
        public RejectSalaryDto payload { get; set; }
    }

    public class RejectTrainingExtraCommandHandler : IRequestHandler<RejectTrainingExtraCommand, Result>
    {
        ...
        public RejectTrainingExtraCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectTrainingExtraCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectTrainingExtra(request.payload);
