...
{
    public class ApprovedBurialCommand : IRequest<Result>
    {
        public ApprovedSalaryDto payload { get; set; }
    }
    public class ApprovedBurialCommandHandler : IRequestHandler<ApprovedBurialCommand, Result>
    {
        ...
        public ApprovedBurialCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(ApprovedBurialCommand request, CancellationToken cancellationToken)
        {
            ...
            if (approver != null)
            {
                ...
                await _staffRequestRepository.ApprovedBurial(request.payload, false);
                ...
                foreach (var item in list)
                {
                    ...
                    string fmt = File.ReadAllText(_appSettings.AceptRequestBurial);
            ...
            {
                await _staffRequestRepository.ApprovedBurial(request.payload, true);

                Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

                string fmt = File.ReadAllText(_appSettings.AceptRequestBurialFin);
