...
{
    public class RejectStaffMedicalCommand : IRequest<Result>
    {
        public RejectSalaryDto payload { get; set; }
    }

    public class RejectStaffMedicalCommandHandler : IRequestHandler<RejectStaffMedicalCommand, Result>
    {
        ...
        public RejectStaffMedicalCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectStaffMedicalCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectMedical(request.payload);

            Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

            string fmt = File.ReadAllText(_appSettings.RejectRequestMedical);
