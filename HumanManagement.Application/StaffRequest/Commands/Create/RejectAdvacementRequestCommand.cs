...
{
    public class RejectAdvacementRequestCommand : IRequest<Result>
    {
        public RejectAdvacementDto payload { get; set; }
    }

    public class RejectAdvacementRequestCommandHandler : IRequestHandler<RejectAdvacementRequestCommand, Result>
    {
        ...
        public RejectAdvacementRequestCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectAdvacementRequestCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectedAdvacement(request.payload);

            Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

            string fmt = File.ReadAllText(_appSettings.RejectRequestAdvacement);
