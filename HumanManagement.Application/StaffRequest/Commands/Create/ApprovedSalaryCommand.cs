...
{
    public class ApprovedSalaryCommand : IRequest<Result>
    {
        public ApprovedSalaryDto payload { get; set; }
    }

    public class ApprovedSalaryCommandHandler : IRequestHandler<ApprovedSalaryCommand, Result>
    {
        ...
        public ApprovedSalaryCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(ApprovedSalaryCommand request, CancellationToken cancellationToken)
        {
            ...
            if (approver != null)
            {
                ...
                await _staffRequestRepository.ApprovedSalary(request.payload, false);
                foreach (var item in list)
                {
                    Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

                    string fmt = File.ReadAllText(_appSettings.AcepteRequestSalaryAcept);
            ...
            {
                await _staffRequestRepository.ApprovedSalary(request.payload, true);
