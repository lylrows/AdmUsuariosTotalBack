...
{
    public class RejectSalaryCommand : IRequest<Result>
    {
        public RejectSalaryDto payload { get; set; }
    }

    public class RejectSalaryCommandHandler : IRequestHandler<RejectSalaryCommand, Result>
    {
        ...
        public RejectSalaryCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(RejectSalaryCommand request, CancellationToken cancellationToken)
        {
            var typeStaffRequest = await baseTypeStaffRequestRepository.Find(request.payload.idTypeStaffRequest);
            await _staffRequestRepository.RejectSalary(request.payload);
