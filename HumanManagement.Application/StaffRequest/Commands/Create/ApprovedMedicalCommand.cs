...
{
    public class ApprovedMedicalCommand : IRequest<Result>
    {
        public ApprovedSalaryDto payload { get; set; }
    }

    public class ApprovedMedicalCommandHandler : IRequestHandler<ApprovedMedicalCommand, Result>
    {
        ...
        public ApprovedMedicalCommandHandler(IBaseRepository<Domain.Notification.Model.NotificationModel> basenotify,
        ...
        public async Task<Result> Handle(ApprovedMedicalCommand request, CancellationToken cancellationToken)
        {
            ...
            if (approver != null)
            {
                ...
                await _staffRequestRepository.ApprovedMedical(request.payload, false);
                foreach (var item in list)
                {
                    Domain.Notification.Model.NotificationModel newNotification = new Domain.Notification.Model.NotificationModel();

                    string fmt = File.ReadAllText(_appSettings.AcepteRequestMedico);
            ...
            {
                await _staffRequestRepository.ApprovedMedical(request.payload, true);
