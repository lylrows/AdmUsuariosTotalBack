...
{
    public class GenerateCvCommand : IRequest<Result>
    {
    ...
    public class GenerateCvCommandHandler : IRequestHandler<GenerateCvCommand, Result>
    {
        ...
        public GenerateCvCommandHandler(IPersonRepository personRepository, 
        ...
        public async Task<Result> Handle(GenerateCvCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!String.IsNullOrEmpty(spathWord))
            {
                ...
                fullpathpdf = string.Format("{0}\\{1}.pdf", _appSettings.TemplateCvPostulantTemp, randomnameNew1);
                ...
                try
                {
                    ...
                    wordDocument = appWord.Documents.Open(fullpath);
                    wordDocument.ExportAsFixedFormat(fullpathpdf, WdExportFormat.wdExportFormatPDF);
                    wordDocument.Close();
                    appWord.Quit();
                }
                catch (Exception ex)
                ...
                {
                    var fileBytes = File.ReadAllBytes(fullpathpdf);
