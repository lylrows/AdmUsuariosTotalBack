...
{
    public class GetInformation2RegisterPostulantQuery: IRequest<Result>
    {
        public int IdPerson { get; set; }

        public class GetInformation2RegisterPostulantQueryHandle : IRequestHandler<GetInformation2RegisterPostulantQuery, Result>
        {
            ...
            private readonly IEvaluationPostulantRepository _evaluationPostulantRepository;

            public GetInformation2RegisterPostulantQueryHandle(IInformationEducationRepository informationEducationRepository,
                                                      ...
                                                      )
            {
                ...
                this._evaluationPostulantRepository = evaluationPostulantRepository;
            }
            public async Task<Result> Handle(GetInformation2RegisterPostulantQuery request, CancellationToken cancellationToken)
            {
                var dto = new InformationPostulantAllDto();

                try 
                {
                    var maxIdEvaluation = _evaluationPostulantRepository.GetMaxIdEvaluationByPostulant(request.IdPerson);
                    if(maxIdEvaluation != 0)
                    {
                        dto.InformationFamilyDtos = await informationFamilyRepository.GetInformationFamily(request.IdPerson, maxIdEvaluation);
                        ...
                        var listInfo = await informationEducationRepository.GetInformationEducation(request.IdPerson, maxIdEvaluation);
                        ...
                        var entityWork = await baseRepositoryWork.FindAllPredicate(x => x.IdPostulant == request.IdPerson && x.IdEvaluation == maxIdEvaluation);
                        ...
                        if (dto.InformationEducationDtos.Count == 0)
                        {
                            ...
                            foreach (var item in educationSitePostulant)
                            {
                                ...
                                newEducation.IdEvaluation = maxIdEvaluation;
                        ...
                        if (dto.InformationWorkDtos.Count == 0)
                        {
                            ...
                            foreach (var item in empleoslist)
                            {
                                ...
                                empleodto.IdEvaluation = maxIdEvaluation;
                            ...
                            foreach (var item in dto.InformationWorkDtos)
                            {
                                ...
                                entity.IdEvaluation = maxIdEvaluation;
                                if (entity.Id == 0)
                                {
                        ...
                        var entitySkills = await baseRepositorySkills.FindAllPredicate(x => x.IdPostulant == request.IdPerson && x.IdEvaluation == maxIdEvaluation && x.Active);
                        dto.InformationComputerSkillsDtos = mapper.Map<List<InformationComputerSkillsDto>>(entitySkills);
                        if (entitySkills.Count == 0)
                        {
                            var _skills = await _utilRepository.GetSkillsPostulant(request.IdPerson, maxIdEvaluation);
                            dto.InformationComputerSkillsDtos = _skills;
                        }

                        var entityAditional = await baseRepositoryAdicional.FindPredicate(x => x.IdPostulant == request.IdPerson && x.IdEvaluation == maxIdEvaluation);
                        dto.InformationAditionalDtos = mapper.Map<InformationAditionalDto>(entityAditional);

                        var entityExtra = await baseRepositoryExtra.FindPredicate(x => x.IdPostulant == request.IdPerson && x.IdEvaluation == maxIdEvaluation);
                        ...
                        var listaDocumentosAdicionales = await _utilRepository.GetDocumentosAdicionales(maxIdEvaluation, request.IdPerson);
                        dto.listaDocumentosAdicionales = listaDocumentosAdicionales;
                        #endregion

                        if (maxIdEvaluation != 0)
                        {
                            var evaluation = await evaluationbaseRepository.FindPredicate(x => x.Id == maxIdEvaluation);
